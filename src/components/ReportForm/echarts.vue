...
<script>
import {getScoreAnalysisByMonth,getCustomizeScoreAnalysisByMonth} from '@/utils/axios/axios'
export default {
  name: 'echarts',
  data () {
    return {
  ...
  methods: {
    ...
    init () {
      if(this.$route.query.departmentId ) {
        ...
        getScoreAnalysisByMonth( this.$qs.stringify(params) ).then(res=>{
          if (res.code ===1) {
            console.log(res,'默认');
            
              this.list=res.data
            // this.chartData = res.data
            let arr = this.$route.query.date.split('-')
            // console.log(  this.chartList ,'定义',this);
            // this.year = this.$route.query.data.split('-')[0]
            this.month = arr[1]
          ...
          // console.log(params,'分析参数',this.$route.query);
          getCustomizeScoreAnalysisByMonth( this.$qs.stringify(params) ).then(res=>{
            if (res.code ===1) {
              console.log(res,'自定义');
