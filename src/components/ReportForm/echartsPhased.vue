...
<script>
import {getScoreAnalysisByStage,getCustomizeScoreAnalysisByStage} from '@/utils/axios/axios'
export default {
  name: 'echarts',
  data () {
    return {
  ...
  methods: {
    ...
    init () {
      if(this.$route.query.departmentId) {
        ...
        getScoreAnalysisByStage( this.$qs.stringify(params) ).then(res=>{
         if (res.code ===1) {
            console.log(res.data,'默认分析');
            this.list=res.data
            // this.chartList = res.data
            // this.chartData = res.data
            // console.log(  this.list ,'定义',this.$route.query);
            let begin = this.$route.query.begin.split('-')
            this.begin = begin[1]
            let end = this.$route.query.end.split('-')
            this.end = end[1]
          ...
          console.log('自定义分析',params);
        
        // console.log(params,'分析参数',this.$route.query);
        getCustomizeScoreAnalysisByStage( this.$qs.stringify(params) ).then(res=>{
           if (red.code ===1 ) {
             console.log(res.data,'自定义分析');
              this.list=res.data
              // this.chartList = res.data
              // this.chartData = res.data
              // console.log(  this.list ,'定义',this.$route.query);
