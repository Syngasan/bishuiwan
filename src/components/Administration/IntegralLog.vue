  ...
  <div class="IntegralLog">
    ...
    <el-table :data="tableData" stripe>
      ...
      <el-table-column prop="createTime" label="操作" align="center" width="300px">
        <template slot-scope="scope">
          <div>
            <span class="tableBtn" @click="steadyIfSettlement(scope.row.uuudate)">重新结算</span>
            <!-- <router-link :to="'/IntegralDetail?month='+scope.row.createTime"></router-link> -->
            <span class="tableBtn" @click="IntegralDetail(scope.row.uuudate)">查看明细</span>
          </div>
          <div>
            <!-- <router-link :to="'/ExclusionStaff?month='+scope.row.createTime"></router-link> -->
            <span class="tableBtn" @click="ExclusionStaff(scope.row.createTime,scope.row.uuudate)">查看排除人员</span>
            <span class="tableBtn" @click="exportExcel(scope.row.createTime,scope.row.uuudate)">导出报表</span>
          </div>
        </template>
        <!-- <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    exportExcel(value, uuudate) {
      console.log(uuudate, "er9rgfdgdfjk8df");
      ...
      console.log(params, "sgfdfsdskkkloioo");
      ...
      exportRewardTask(this.$qs.stringify(params)).then(res => {
        if (res.code === 0) {
          ...
          let str =
            "http://47.107.71.47/bishuiwan/api/steady/exportSteady?sessionId=" +
      ...
      this.$axios({
        ...
        if (res.data.code === 1) {
          ...
          for (let item of this.tableData) {
            let arr = item.createTime.split(" ")[0];
            item.uuudate = item.createTime.split(" ")[0];
            item.createTime = arr.split("-")[0] + "-" + arr.split("-")[1];
